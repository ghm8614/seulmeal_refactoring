...
<body>
	...
	<div class="container">
		...
		<div class="row"
			style="border-top: thick double #ff4500; border-bottom: thick double #ff4500; margin-bottom: 1rem; margin-top: 2rem;">
			<div class="col-6">
				<h1 class="pt-5 mb-4 fw-bold">신고게시글</h1>
			</div>
			<div class="dropdown pt-5 mb-4 col-6"></div>
		</div>
		<table class="table table-hover text-center">
			...
			<tbody>
				<c:if test="${reportList.size() == 0}">
					<tr>
						<td colspan="5" style="font-size: 100px;"><i
				...
				<c:forEach var="report" items="${reportList}">
					<tr>
						...
						<th class="postNo"><a
							href="/api/v1/community/post/reports/${report.postNo}"
							class="link-dark text-decoration-none">${report.postNo}</a></th>
						<th>${report.reason}</th>
						<th class="status" style="font-weight: bolder;"><c:if
								test="${report.status==0}">
							답변전
						</c:if> <c:if test="${report.status==1}">
							답변완료
						</c:if></th>
			...
			<nav aria-label="...">
				<ul class="pagination">
					<li class="page-item disabled"><span class="page-link"
						data-value="1">Previous</span></li>
					<c:forEach var="i" begin="${resultPage.beginUnitPage}"
						end="${resultPage.endUnitPage}">
						<li class="page-item"><c:if
								test="${search.searchKeyword ==null }">
								<a class="page-link" href="/community/getListReportPost/${i}">${i}</a>
							</c:if> <c:if test="${search.searchKeyword !=null }">
								<a class="page-link"
									href="/community/getListReportPost/${i}?searchKeyword=${search.searchKeyword}">${i}</a>
							</c:if></li>
					</c:forEach>
